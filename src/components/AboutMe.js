import React from "react";

function AboutMe() {
  return (
    <div>
      <hr />
      <h1 className="font-bold text-[1.6rem] text-center font-Inter">About me </h1>
      <p className="font-Lora m-4 text-lg">
        In a world cluttered with distractions, I believe in the power of
        simplicity to cut through the noise and deliver meaningful experiences. 
        As a full stack web developer, I bring together form and functions in a
        harmonious balance, crafting minimalist websites that speak volumes with
        subtlety. With a keen eye for detail and a love for elegant simplicity, I
        bring your vision to life by combining technical expertise with a
        refined aesthetic, ensuring every pixel serves a purpose and every
        interaction is seamless.
      </p>
    </div>
  );
}

export default AboutMe;
